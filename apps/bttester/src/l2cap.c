...
static struct channel *get_free_channel() {
	...
	console_printf("Get free channel\n");
...
static void connect(u8_t *data, u16_t len) {
	...
	console_printf("connect: type: %d addr: %s\n", addr->type, bt_hex(addr->val, 6));

	rc = ble_gap_conn_find_by_addr(addr, &desc);
	if (rc) {
		console_printf("GAP conn find failed\n");
		goto fail;
	}

	chan = get_free_channel();
	if (!chan) {
		console_printf("No free channels");
		goto fail;
	}

	sdu_rx = os_mbuf_get_pkthdr(&sdu_os_mbuf_pool, 0);
	if (sdu_rx == NULL) {
		console_printf("Failed to alloc buf");
		goto fail;
	}
	...
	if (rc) {
		console_printf("L2cap connect failed\n");
		goto fail;
	}
...
static void send_data(u8_t *data, u16_t len) {
	...
	SYS_LOG_ERR("cmd->chan_id=%d", cmd->chan_id);
