...
cmd_l2cap_reconfig(int argc, char **argv)
{
    ...
    mps = parse_arg_uint16_dflt("mps", 0, &rc);
    if (rc != 0) {
       console_printf("invalid 'mps' parameter\n");
       return rc;
    }

    rc = parse_arg_byte_stream("idxs", 5, idxs, &num);
    if (rc != 0) {
       console_printf("invalid 'idxs' parameter\n");
       return rc;
    }

    return btshell_l2cap_reconfig(conn, mtu, mps, num, idxs);
#else
    return ENOTSUP;
#endif
}
